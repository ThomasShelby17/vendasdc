...
module.exports = {
    ...
    run: async(client, message, args) => {
      ...
      if(user === `${config.get(`owner`)}`) return message.reply(`<a:no:1115828260920635392>・Essa pessoa já é o dono do bot!`).then(msg => setTimeout(() => msg.delete().catch(err => console.log(err)), 5000));
      if(isNaN(args)) return message.reply(`<a:no:1115828260920635392>・Você só pode adicionar IDs!`).then(msg => setTimeout(() => msg.delete().catch(err => console.log(err)), 5000));
        
      message.reply(`<a:sim:1115828258676678747>・Permissão de dono adicionada!`).then(msg => setTimeout(() => msg.delete().catch(err => console.log(err)), 5000));
