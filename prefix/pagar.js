...
module.exports = {
    ...
    run: async(client, message, args) => {
      ...
      const id = args[0]
      dbP.set(`pay.emoji`, `<a:pretobranco:1115827538263023757>`)
        dbP.set(`pay.nome`, `Setado Como Pago`)
      db3.set(`${id}.status`, `Processando`).then(msg => setTimeout(() => msg.delete().catch(err => console.log(err)), 5000));
      message.reply("<a:planeta:1115828269774819348>・Status de compra alterado para pago!").then(msg => setTimeout(() => msg.delete().catch(err => console.log(err)), 5000));
    }
}